...
\node[block, below=0.35cm of graph] (reader) {Contrastively fine-tuned\\VLM reader (\S\ref{sec:contrastive})};
\node[block, below=0.35cm of reader] (ctrl) {Evidence controller\\(budgeted, \S\ref{sec:controller})};
\draw[arr] (render) -- (tile);
\draw[arr] (tile) -- (graph);
\draw[arr] (graph) -- (reader);
\draw[arr] (reader) -- (ctrl);
\end{tikzpicture}
\caption{Pipeline overview. Rendering, tiling, and graph construction happen once per page; the reader is fine-tuned once, offline; the evidence controller runs once per query, deciding which graph nodes the reader opens.}
\label{fig:pipeline}
\end{figure}
...
\paragraph{Oversized-page splitting.} One article's screenshot exceeded $46{,}000$\,px, producing dozens of tiles whose sequential QA generation (\S\ref{sec:contrastive}) ran for hours with no partial progress saved on failure. Rather than exclude such pages, one whose element height exceeds $10\times$ the max tile height is split into sections \emph{before} tiling, at \code{h1}/\code{h2} boundaries, with any still-oversized section further re-chunked by a height budget. Each section is tiled and graphed as a standalone page would be; the per-section graphs are merged by node-namespacing, with consecutive sections' \code{page} nodes linked by a new \textbf{\code{continues}} relation -- sequential parts of the same document, distinct from the hyperlink relation \code{links\_to}. Every page's graph, split or not, exposes an \emph{entry point} for the corpus graph below to rewire onto. Figure~\ref{fig:section-split} (Appendix~\ref{sec:appendix-figures}) shows an example split into 16 sections.
...
Given a query, the controller decides which element nodes the reader actually reads, rather than the whole page or a static top-$k$. It evolves each node's \code{state} through a four-state machine (\state{inactive}$\to$\state{active}$\to$\state{opened}/\state{pruned}, Appendix~\ref{sec:appendix-figures}, Figure~\ref{fig:states}), generalizing MAGE-RAG's Algorithm~1.
...
\paragraph{Policy.} Algorithm~\ref{alg:controller}: (1) seed the frontier with the $k$ highest-scoring inactive nodes; (2) repeatedly take the highest-scoring \emph{active} node, pruning it for free below an opening threshold or opening it (one unit of budget $B$) and activating still-inactive neighbors that clear an activation threshold; (3) once budget is exhausted or no active node remains, prune whatever is left active. Neighbors are only considered \emph{after} their node is opened, conditioning the frontier on what has already proven relevant rather than a fixed global ranking -- what distinguishes the controller from static top-$k$ retrieval over the same scores (ablation (c)). ``Neighbor'' includes \code{links\_to}/\code{continues} targets, not just same-page \code{reading\_order}: the fix that lets the frontier reach a second document at all, instead of only through the initial seed (\S\ref{sec:experiments}).
...
\While{$b < B$ and some node is \state{active}}
  ...
  \Else
    ...
    \For{each inactive neighbor $n'$ of $n$ via \code{reading\_order}, \code{links\_to}, or \code{continues}}
