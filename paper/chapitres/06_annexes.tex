...
SQuAD~\citep{squad2016}, et par ANLS (\emph{Average Normalized Levenshtein
Similarity}, \code{scripts/eval\_multihop\_e2e.py::\_anls}), le standard du
benchmark DocVQA~\citep{docvqa2021} pour les systèmes de question-réponse
\emph{visuels}~: un système qui lit des pixels plutôt que du texte extrait par
OCR est sujet à de petites erreurs de forme (un caractère de la réponse
produite par le VLM manquant ou substitué) que la correspondance exacte
sanctionne au même titre qu'une réponse sans rapport, alors qu'ANLS accorde un
crédit proportionnel à la distance d'édition normalisée (nul en-deçà d'un
seuil de similarité de \num{0.5}, pour ne pas non plus créditer une réponse
conceptuellement fausse). Les trois métriques sont rapportées côte à côte~:
EM/$F_1$ pour la comparabilité avec HotpotQA, ANLS pour la comparabilité avec
DocVQA~\citep{docvqa2021}, LongDocURL~\citep{longdocurl2024} et
MMLongBench-Doc~\citep{mmlongbenchdoc2024} --- les deux derniers étant les
benchmarks documentaires sur lesquels MAGE-RAG~\citep{magerag2026} est évalué
(\cref{chap-resultats}) --- et pour distinguer, précisément plutôt que par
inspection manuelle seule, un échec de raisonnement d'un simple bruit de
forme.
...
\textbf{L'ANLS ne change pas ce diagnostic.} À \num{0.050}, elle est
quasiment identique à la correspondance exacte, alors que c'est justement sur
l'écart entre les deux que repose son intérêt~: si les erreurs du système
étaient du bruit de forme --- une réponse par ailleurs correcte, légèrement
mal formatée ou tronquée par le VLM ---, ANLS aurait recrédité une partie de
ce que la correspondance exacte ramène à zéro, et l'écart entre les deux
scores mesurerait directement l'ampleur de cet artefact de métrique.
L'absence quasi totale d'écart est donc elle-même un résultat~: sur cet
échantillon, la métrique intolérante n'est pas ce qui déprime le score, ce que
confirme la \cref{sec-res_d_erreurs}~: les prédictions manquées sont pour la
plupart des refus textuels entiers (« Not specified in evidence ») ou des
entités clairement différentes de la référence, jamais des quasi-réponses à
un caractère près --- la distance d'édition normalisée à la référence y reste
loin sous le seuil de crédit partiel de ANLS (\num{0.5}) dans la quasi-totalité
des cas. Le sous-rappel de preuves documenté ci-dessous, pas l'intolérance de
la métrique, porte donc l'essentiel de l'échec observé.
...
de la partie hors ligne de la chaîne, extraits du module de configuration.}
...
Budget de hauteur d'une section trop haute & \SI{20480}{\pixel} \enspace(même seuil que la ligne précédente) & \cref{sec-tuilage_sections} \\
...
Pages suivies depuis l'amorce ($m$) & \num{3} & \cref{sec-graphe_corpus} \\
...
Plage de $n$-grammes & \num{1}--\num{1} \enspace(unigrammes) & \cref{sec-controleur_scoreur} \\
Liste de mots vides & \code{"english"} (scikit-learn) & \cref{sec-controleur_scoreur} \\
Fréquences documentaires minimale et maximale & \num{1} / \num{1.0} \enspace(pas de borne supérieure) & \cref{sec-controleur_scoreur} \\
Pondération sous-linéaire des fréquences & non & \cref{sec-controleur_scoreur} \\
Longueur de l'aperçu textuel par tuile & \num{200} caractères & \cref{sec-tuilage} \\
\bottomrule
\end{tabularx}
\end{table}
...
Paramètres entraînables & \num{44302336} \enspace(\SI{0.63}{\percent} du total) & \cref{sec-contrastif_lora} \\
Optimiseur, taux d'apprentissage, échauffement & \code{AdamW}, \num{1e-4}, aucun & \cref{sec-contrastif_lora} \\
...
Aire d'image minimale du processeur (\code{min\_pixels}) & \SI{3136}{\pixel} & \cref{sec-contrastif_memoire} \\
Aire d'image maximale du processeur (\code{max\_pixels}) & \SI{12845056}{\pixel} & \cref{tab-cout_tokens} \\
