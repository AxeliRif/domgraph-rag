...
d'éléments extraits. L'unité la plus dense du corpus, une section de l'article
consacré aux Beatles, en compte \num{161} avant élagage. La chaîne des
comptages médians par unité de page s'établit à
...
celles de la tour de vision (\code{qkv}, \code{proj}). Le suffixe générique
\code{proj}, pensé pour la tour de vision, matche par construction tout nom de
module s'y terminant~; il capture donc aussi, en plus des projections
d'attention visées, les projections MLP du décodeur (\code{gate\_proj},
\code{up\_proj}, \code{down\_proj}). L'adaptation couvre ainsi une portée plus
large que l'attention seule, ce que reflète le compte de paramètres
entraînables ci-dessus. Juger si une tuile répond
